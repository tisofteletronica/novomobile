...
export async function getProductsLockByActuator(automaker: string) {
  const { data } = await httpClient.get<ProductsUniqueByAutomakerResponse[]>(
    `/products/atuadorProdutoMontadora?montadora=${automaker}`,
  );
