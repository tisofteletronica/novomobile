...
export async function getProductsByModelAndCategory(
  model: string,
  year: string,
  category: string,
  page?: number | string,
  ...
  const { data } = await httpClient.get<ProductsByModelAndCategoryResponse>(
    `/app-instalesoft/modeloprodutos?ano=${year}&categoriaId=${category}&modeloId=${model}&page=${page}&size=${size}`,
  );
